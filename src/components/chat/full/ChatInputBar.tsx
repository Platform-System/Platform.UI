...
const ChatInputBar: React.FC<ChatInputBarProps> = ({ message, setMessage, onSend }) => {
  return (
    /* Thanh soạn thảo tin nhắn cố định dưới cùng của trang Chat */
    <div className="p-5 bg-[#0a0a0f]/80 backdrop-blur-xl border-t border-white/5 shrink-0 z-10">
      <form onSubmit={onSend} className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full p-1.5 pr-2 focus-within:border-cyan-500/50 focus-within:shadow-[0_0_20px_rgba(34,211,238,0.1)] transition-all">
        {/* Nút Emoji */}
        <button title="Emojis" type="button" className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-pink-500 rounded-full hover:bg-white/10 transition-colors shrink-0">
          <Icon icon="solar:smile-circle-linear" width="24" />
        </button>
        {/* Ô nhập liệu văn bản chính */}
        <input 
          type="text" 
          value={message}
        ...
        <button title="Attach Files" type="button" className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-cyan-400 rounded-full hover:bg-white/10 transition-colors shrink-0 mr-1">
